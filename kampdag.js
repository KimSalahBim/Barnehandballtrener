...
// Barnehandballtrener - kampdag.js
...
(function () {
  ...
  function updateKeeperSummary() {
    ...
    if (format === 4) {
      summary.textContent = '4-er: keeperrollen roteres automatisk mellom alle spillerne.';
      return;
    }
  ...
  function swapBenchToField(si, benchPid, fieldSlot) {
    ...
    if (currentFormat() === 4 && fieldPid && fieldPid === keeper4(si)) kdRotatingKeepers[si] = benchPid;
  ...
  function copySlotToNext(si) {
    ...
    if (currentFormat() === 4) { const kk = keeper4(si); if (kk && Object.values(tgt.slots).includes(kk)) kdRotatingKeepers[si + 1] = kk; }
    renderKampdagOutput(lastPresent, lastBest, lastP, lastT);
  }
